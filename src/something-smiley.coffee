...
fbs.on 'value', (ssnap) ->
  ...
  $('table.post').each ->
    postfb = fb.child(@id)
    smileybox = $("<dd class='smilies'><h6>Smiley Replies</h6></dd>").appendTo($(@).find('dl.userinfo'))
    formbtn.clone().click(btnclick).appendTo("<li></li>").appendTo($(@).find(".postbuttons"))

    postfb.on "child_added", (snap, uname) ->
